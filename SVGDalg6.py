...
data=torch.vstack((torch.tensor(np.linspace(-10,10,1000)),torch.exp(dist_q.log_prob(torch.tensor(np.linspace(-10,10,1000))))))
data=torch.vstack((data,torch.exp(dist_p.log_prob(torch.tensor(np.linspace(-10,10,1000))))))
...
for l in tqdm.tqdm(range(n_iterations)):
    ...
    h=float(torch.median(torch.sort(torch.flatten(torch.flatten(torch.triu(torch.abs(x_difference)))))[0][int((n**2+n)/2):]))**2/np.log(n)
    #k[i,j]=k(x_j,x_i)
    ...
    #grad_p[i,j]=\nabla_x p(x_j)
    grad_p=(0.3/(np.sqrt(2*np.pi))*(-x_ntimes-2.5)*torch.exp(-torch.pow(x_ntimes+2.5,2)/2)+0.7/(np.sqrt(2*np.pi))*(-x_ntimes+1.5)*torch.exp(-torch.pow(x_ntimes-1.5,2)/2))*torch.pow(p,-1)
    phi_summand=1/n*(grad_p*k+grad_k)*torch.pow(q,alpha-1)*torch.pow(p,1-alpha)
    #phi
