...
#print(m.log_prob(torch.zeros(2)))
h=10.0
epsilon=1
alpha=1   
...
for l in range(25):
    ...
    #x5[i,j]=k_grad(xj,xi)
    #x6=-x2
    x6=torch.exp(torch.sum((-0.5*torch.pow(x2,2)),0))/(2*np.pi)
    x6=torch.mul(-x2,torch.stack([x6,x6],dim=0))
    #x6[:,i,j]=logp_grad(xj)
    #x7=1/n**2*x4*x6+1/n**2*x5
    #x7=1/n**2*(x6*x4+(torch.exp(torch.sum((-0.5*torch.pow(x2,2)),0))/(2*np.pi))*x5)*torch.pow(torch.exp(torch.sum((-0.5*torch.pow(x2,2)),0))/(2*np.pi),alpha-1)/(1/25**alpha)
    x7=1/n**2*(x6*x4+(torch.exp(torch.sum((-0.5*torch.pow(x2,2)),0))/(2*np.pi))*x5)*torch.pow(25*torch.exp(torch.sum((-0.5*torch.pow(x2,2)),0))/(2*np.pi),alpha-1)/(1/25)
    #x7[:,i,j]=Phi(xi,xj)
    x8=torch.sum(x7,2)
    #x8[:,i]=Phi(xi)
    x=x+epsilon*x8
